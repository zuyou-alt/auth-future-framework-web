...
<template>
  ...
  <div class="card table-main">
    ...
    <!-- 表格主体 -->
    <el-table :height="tableHeight" ref="tableRef" v-bind="$attrs" :data="processTableData" :border="border" :row-key="rowKey" @selection-change="selectionChange">
      <!-- 默认插槽 -->
      <slot />
      <template v-for="item in tableColumns" :key="item">
        ...
        <!-- other -->
        <TableColumn v-if="!item.type && item.prop && !item.isHide" :column="item">
...
const colSetting = tableColumns!.filter(item => {
  const { type, prop, isHide } = item;
  return !columnTypes.includes(type!) && prop !== "operation" && !isHide;
});
