<template>
  <div class="message">
    <el-popover :visible="popoverShow" @update:visible="updateVisible" placement="bottom" :width="310" trigger="click">
      <template #reference>
...
import { ref } from "vue";
import { Message } from "@/api/interface/wsMessage";
const activeName = ref("first");
const popoverShow = ref(false);
const personalMessages = ref<Message.MessageInfo[]>();
/**
 * 控制消息组件显示还是关闭
 */
...
const messageHandler = (data: Message.MessageInfo) => {
  console.log("消息组件收到消息", data);
  popoverShow.value = true;
  personalMessages.value?.push(data);
};
